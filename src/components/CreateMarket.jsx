...
export function CreateMarket({ contractHook, onMarketCreated, selectedFixture }) {
  ...
  return (
    <div className="glass-card rounded-xl border border-white/10 p-5 sm:p-8">
      <div className="mb-6 sm:mb-8">
        <div className="mb-2 flex items-center gap-3">
          ...
          <h2 className="text-xl font-bold tracking-tight text-white sm:text-2xl">Create New Market</h2>
        </div>
        <p className="text-sm text-gray-400 sm:pl-[3.25rem]">
          {selectedFixture ? 'Pre-filled from selected fixture' : 'Enter match details manually'}
        </p>
      </div>

      {selectedFixture && (
        <div className="mb-6 rounded-xl border border-info/30 bg-info/10 p-4 animate-scale-in">
          <div className="flex items-start gap-3">
            <svg className="mt-0.5 h-5 w-5 flex-shrink-0 text-info" fill="currentColor" viewBox="0 0 20 20">
              <path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clipRule="evenodd" />
            </svg>
            <div className="flex-1">
              <p className="mb-1 text-sm font-medium text-info">Fixture Selected</p>
              <p className="text-xs text-info/70">
                {selectedFixture.team1} vs {selectedFixture.team2} <span className="px-1">&middot;</span> {selectedFixture.league}
              </p>
            </div>
          </div>
        </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          <div>
            <label className="mb-3 block text-xs font-medium uppercase tracking-wider text-gray-500">Home Team</label>
            <div className="relative">
              ...
              <input
                ...
                className="w-full rounded-lg border border-white/10 bg-white/5 py-3.5 pl-12 pr-4 text-white placeholder-gray-600 transition-all duration-200 focus:border-white/30 focus:bg-white/10 focus:outline-none"
                disabled={isCreating}
              />
            </div>
          </div>
          ...
          <div>
            <label className="mb-3 block text-xs font-medium uppercase tracking-wider text-gray-500">Away Team</label>
            <div className="relative">
              <div className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500">
                <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div>
          <label className="mb-3 block text-xs font-medium uppercase tracking-wider text-gray-500">League / Competition</label>
          <div className="relative">
            <div className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500">
        ...
        <div>
          <label className="mb-3 block text-xs font-medium uppercase tracking-wider text-gray-500">Match Date & Time</label>
          <div className="relative">
            <div className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500">
              <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <input
              ...
              className="w-full rounded-lg border border-white/10 bg-white/5 py-3.5 pl-12 pr-4 text-white transition-all duration-200 focus:border-white/30 focus:bg-white/10 focus:outline-none"
              disabled={isCreating}
            />
          </div>
        </div>
        ...
        <div>
          <label className="mb-3 block text-xs font-medium uppercase tracking-wider text-gray-500">Resolution Source URL</label>
          <div className="relative">
            <div className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500">
              <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <label className="group flex cursor-pointer items-start gap-3">
            <div className="relative mt-0.5 flex items-center justify-center">
              <input
                ...
                disabled={isCreating}
              />
              <div className="h-6 w-11 rounded-full bg-white/10 transition-all duration-200 peer-checked:bg-success" />
              <div className="absolute left-0.5 top-0.5 h-5 w-5 rounded-full bg-white transition-transform duration-200 peer-checked:translate-x-5" />
            </div>
            <div className="flex-1">
